...
public class GenAiService {
    ...
    @Autowired
    public GenAiService(@Value("${google.genai.api-key}") String apiKey) {
        this.client = new Client.Builder().apiKey(apiKey).build();
    }

    /**
     * Summarize website content into a detailed summary.
     */
    public String generateResponseFromContent(String websiteContent) {
        try {
            ...
            GenerateContentResponse response = client.models.generateContent("gemini-2.0-flash-001", prompt.toString(), null);
    ...
    /**
     * Generate a personalized cold outreach email based on lead info and product prompt.
     */
    public String generateResponse(String query, String prompt1, String name, int userId) {
        try {
            Setting s = setting.findByUserId(userId)
            ...
            String prompt = "Write a personalized Sales outreach Cold email to the lead " + name + " for our product.\n" +
                    ...
                    "From the addressed painpoints as provided use 1-2 differentiators.\n" +
                    "To stay Relevant connect to their products/industry if data exists.\n" +
    ...
    /**
     * Generate a cold email based on company name, website URL, and website content.
     */
